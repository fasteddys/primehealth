...
namespace WebApplication1.BLL
{
    public class Accounts
    {
        ...
        public bool LogiAuth(string name, string pass, ref string type, ref int id)
        {
            try
            {
                var data = from p in _db.userTBs where p.UserName == name select p;
        ...
        #region add user
        public bool adduser(string name, string pass, string type,string Mail)
        {
        ...
        public bool adduser_ForAdmin(string name, string pass, string type)
        {
            bool IsExists = _db.userTBs.Any(pa => pa.UserName.Equals(name));
            if (IsExists)
            {
        ...
        public IEnumerable<userTB> GetData()
        {
            List<userTB> data = (from p in _db.userTBs
                                 ...
                                 where p.Type == "Archiving" || p.Type == "ArcAdmin" || p.Type == "Admin"
                                 select p).ToList();
            return data;
        }

        public IEnumerable<userTB> GetDataForEnterprise()
        {
            List<userTB> data = (from p in _db.userTBs
                                 orderby p.id ascending
                                 select p).ToList();
        ...
        public IEnumerable<userTB> GetAllUsers()
        {
            List<userTB> data = (from p in _db.userTBs
                                 where p.Type=="Archiving" || p.Type=="ArcAdmin" || p.Type=="Admin"
                                 select p).ToList();
        ...
        public IEnumerable<userTB> GetAllUsersforadmin()
        {
        ...
        public IEnumerable<userTB> GetAllUsersforadminArchiving()
        {
