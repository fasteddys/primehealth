...
namespace CallCenterNotesApp.BLL
{
    public class ClientNotesClass
    {
        ...
        public void addClientNotes(string RequestDate, string ClientNamestr, string ClientTypestr, string Notes, string AddClientNotesName, HttpPostedFile files, System.Web.UI.WebControls.FileUpload FileUpload1)
        {
            ...
            db.SaveChanges();
            helper.NoteFileUpload(files, FileUpload1, req.ClientID);
        }
        ...
        public void UpdateClientNotes(int id, string CName, string CNotes, HttpPostedFile files, System.Web.UI.WebControls.FileUpload FileUpload1)
        {
            ClientNote acc = db.ClientNotes.Single(p => p.ClientID == id);
            acc.ClientName = CName;
            acc.Notes = CNotes;
            db.SaveChanges();
            helper.NoteFileUpload(files, FileUpload1, acc.ClientID);
        }
        ...
        public void DeleteClient(int id)
        {
            ...
            var noteAttachment = db.NoteAttachments.Where(x => x.NoteID == id).ToList();
            foreach (var items in noteAttachment)
            {
                items.IsDeleted = true;
                db.NoteAttachments.AddOrUpdate(items);
            }
            db.SaveChanges();
        }
