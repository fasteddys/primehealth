...
namespace BNC.DAL.Repositories
{
    public class Repository<TT> : ContainerContextFactory, IDisposable, IRepository<TT> where TT : class
    {
        ...
        /// Empty Constructor 
        /// </summary>
        public Repository(BNCEntities Context)
        {
            context = Context;
        ...
        public virtual void Add(TT t)
        {
            try
            {
                //context.Set<TT>().AddOrUpdate(t);
                context.Set<TT>().Add(t);
            }
        ...
        public async Task<IQueryable<TT>> GetAllAsync()
        {
            try
            {
                ...
                var items = await Table.AsNoTracking().ToListAsync();
        ...
        public virtual int Count(Expression<Func<TT, bool>> @where)
        {
            return Table.AsNoTracking().Count(@where);
        }
      ...
      public  IEnumerable<TT> CustomSelection(string fields, Expression<Func<TT, bool>> @where)
        {        
            // input parameter "o"
            var xParameter = Expression.Parameter(typeof(TT), "o");

            // new statement "new Data()"
            var xNew = Expression.New(typeof(TT));
            ...
            // property "Field1"
            var mi = typeof(TT).GetProperty(o);
            ...
            var lambda = Expression.Lambda<Func<TT, TT>>(xInit, xParameter);

            // compile to Func<Data, Data>
            return Table.Where(@where).AsNoTracking().Select( lambda.Compile());
        }
        #endregion
    }
}
