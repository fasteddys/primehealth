...
namespace TickitingSystem.Controllers
{
    ...
    public class TicketController : Controller
    {
        ...
        public JsonResult AddAttachJson()
        {
            int? t = ticketid;
            int? d = detailsid;
            string strMappath = "//10.1.1.15/Backups/Developers Backup/19-Ticketing System/TicketingSystem Attachment/" + t + "/" + d + "/";
            ...
            string staticpath = @"\\10.1.1.15\Backups\Developers Backup\19-Ticketing System\TicketingSystem Attachment\" + t + @"\" + d + @"\";
        ...
        public ActionResult ViewAllTickets(int id)
        {
            ...
            SubTyspe_DAL subtype = new SubTyspe_DAL();
            Permission_DAL permission = new Permission_DAL();

            var listOFSubtype =subtype.GetAllSubTyspe();
            List<int> listofpermissons=new List<int>();
            foreach(var item in listOFSubtype)
            {if (permission.CheckIfUserHasPermission(item.Name))

                {
                    listofpermissons.Add(item.ID);

                }
            }

            if (listofpermissons.Count() !=0 )
            {
                    var tickets = ticketDAL.GetAllTicketsByUserIDwitfiltreation(id, listofpermissons, user.User_ID).OrderByDescending(x => x.ID);
                    return View(tickets);
        ...
        public ActionResult ViewTicketsbyStatus(int typeID, int StatusID)
        {
            ...
            if (user.Dept_ID == 1)
            {
                var tickets = ticketDAL.GetAllTicketsByByTypeIDAndStatuswitfiltreation(typeID, StatusID, listofpermissons, user.User_ID).OrderByDescending(x => x.ID);
        ...
        public JsonResult EditTicketStatus(DAL.DataBase.Ticket updatedticket)
        {
            ...
            if (updatedticket.Status_ID == 2)
            {
                ...
                ticketdetailsDAL.updatelastdetails(updatedticket.ID, updatedticket.Assign_Person_ID);
