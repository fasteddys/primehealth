...
namespace EmaTours.Utilities
{
    ...
    public static class ExceptionHandlerConstants
    {
        ...
        public static void CreateEmaToursException(Exception e, string className, string methodname,string ErrorSource)
        {
            CustomException ex = CreateCustomException(className, methodname, e);
            var connectionstring = ConfigurationManager.AppSettings["EmaToursConnectionString"].ToString();

            using (SqlConnection con = new SqlConnection(connectionstring))
            {
                string query = "INSERT INTO [dbo].[FailedTransactions] (LogDetails, LogType,ClassName,MethodName,LogDate,ErrorSource) VALUES (@LogDetails, @LogType,@ClassName,@MethodName,@LogDate,@ErrorSource)";
                SqlCommand cmd = new SqlCommand(query, con);
                cmd.Parameters.Add("@LogDetails", SqlDbType.VarChar).Value = PrepareExceptionString(ex);
                cmd.Parameters.Add("@LogType", SqlDbType.VarChar, 200).Value = ExceptionHandlerConstants.ExceptionLogType;
                cmd.Parameters.Add("@ClassName", SqlDbType.VarChar).Value = ex.exceptionClass;
