    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dataGridView1.Sort(this.dataGridView1.Columns["F20"], ListSortDirection.Ascending);
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
                    ...
                    for (int si = start - 1; si <= finish - 1; si++)
                    {
                        ...
                        removdup("F20", comboBox5);
                            ...
                            try
                            {
                                if (radioButton8.Checked == true)
                                {
                                    xlbodArch(textBox4.Text + regex.Replace(comboBox2.Items[si].ToString(), ".")+ ".xlsx");

                                }
                                else if (radioButton7.Checked == true)
                                {
                                    xlbod(textBox4.Text + regex.Replace(comboBox2.Items[si].ToString(), ".") + ".xlsx", si, comboBox6.Text, (0.043 / 0.8), comboBox2, Path, prvider_name, comboBox2.Items[si].ToString(), comboBox7.Text);
            ...
            {
                string Path = "\\" + "\\Androiddb\\Attached\\";
        ...
        private void xlbod(String x, int c_index, string type, Double eza_no, ComboBox co, string path, string provider_name, string drop1, string InOutType)
        {
            try
            {
                ...
                if (radioButton6.Checked == true)
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        string stament = ("insert into [TPASys].[dbo].[Provider] (PName,TottalClaims,AttachPath,PMonth,PYear,ClientName,PolicyNumber,PType) values(N'" + provider_name + "','" + claim_count + "','" + path + "','" + ComboBox4.Text + "','" + ComboBox3.Text + "','" + type + "','" + "NULL" + "','" + InOutType + "')");
                        SqlCommand cmd = new SqlCommand(stament);
                        cmd.CommandType = CommandType.Text;
                        cmd.Connection = connection;
                        connection.Open();
