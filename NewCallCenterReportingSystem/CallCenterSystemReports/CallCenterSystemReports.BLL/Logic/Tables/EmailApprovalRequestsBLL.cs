    ...
    public class EmailApprovalRequestsBLL : Repository<EmailApprovalRequest>
    {
        ...
        public List<TopLeastUsersDTO> GetTopAssignees(string UserType, int NumberOfLeastToTake)
        {
            try
            {
                ...
                {
                    var Result = GetAll().Where(p => p.RequstStatusID != (int)RequestStatus.AssignedByOpeartor).Where(p => p.OperatorAssignee != null).GroupBy(p => p.OperatorAssignee).Select(group => new { Asignee = group.Key, TicketCount = group.Count() }).OrderByDescending(p => p.TicketCount).Take(NumberOfLeastToTake).ToList();
                ...
                {
                    var Result = GetAll().Where(p => p.RequstStatusID != (int)RequestStatus.AssignedByDoctor).Where(p => p.DoctorAssignee != null).GroupBy(p => p.DoctorAssignee).Select(group => new { Asignee = group.Key, TicketCount = group.Count() }).OrderByDescending(p => p.TicketCount).Take(NumberOfLeastToTake).ToList();
                    foreach (var item in Result)
                    {
                        TopLeastUsersDTO temp = new TopLeastUsersDTO
                        {
                ...
                {
                    var Result = GetAll().Where(p => p.RequstStatusID != (int)RequestStatus.AssignedByAudit).Where(p => p.AuditAssignee != null).GroupBy(p => p.AuditAssignee).Select(group => new { Asignee = group.Key, TicketCount = group.Count() }).OrderByDescending(p => p.TicketCount).Take(NumberOfLeastToTake).ToList();
                    foreach (var item in Result)
                    {
                        TopLeastUsersDTO temp = new TopLeastUsersDTO
                        {
        ...
        public List<TopLeastUsersDTO> GetLeastAssignees(string UserType, int NumberOfTopToTake)
        {
            try
            {
                ...
                if (UserType == "CallCenterUser")
                {
                    var Result = GetAll().Where(p => p.RequstStatusID != (int)RequestStatus.AssignedByOpeartor).Where(p => p.OperatorAssignee != null).GroupBy(p => p.OperatorAssignee).Select(group => new { Asignee = group.Key, TicketCount = group.Count() }).OrderBy(p => p.TicketCount).Take(NumberOfTopToTake).OrderByDescending(p => p.TicketCount).ToList();
                    foreach (var item in Result)
                    {
                        TopLeastUsersDTO temp = new TopLeastUsersDTO
                        {
                ...
                {
                    var Result = GetAll().Where(p => p.RequstStatusID != (int)RequestStatus.AssignedByDoctor).Where(p => p.DoctorAssignee != null).GroupBy(p => p.DoctorAssignee).Select(group => new { Asignee = group.Key, TicketCount = group.Count() }).OrderBy(p => p.TicketCount).Take(NumberOfTopToTake).OrderByDescending(p => p.TicketCount).ToList();
                    foreach (var item in Result)
                    {
                        TopLeastUsersDTO temp = new TopLeastUsersDTO
                        {
                ...
                {
                    var Result = GetAll().Where(p => p.RequstStatusID != (int)RequestStatus.AssignedByAudit).Where(p => p.AuditAssignee != null).GroupBy(p => p.AuditAssignee).Select(group => new { Asignee = group.Key, TicketCount = group.Count() }).OrderBy(p => p.TicketCount).Take(NumberOfTopToTake).OrderByDescending(p => p.TicketCount).ToList();
                    foreach (var item in Result)
                    {
                        TopLeastUsersDTO temp = new TopLeastUsersDTO
                        {
        ...
        public List<RequestsCountOverDayDTO> GetRequestsCountOverDay()
        {
            ...
            var Result = GetAll().Where(p => p.CreationDate.Value.Year == DateTime.Now.Year && p.CreationDate.Value.Month == DateTime.Now.Month && p.CreationDate.Value.Day == DateTime.Now.Day).GroupBy(p => p.CreationDate.Value.Hour).Select(group => new { Hours = group.Key, RequestsCount = group.Count() }).OrderBy(p => p.Hours).ToList();
            ...
            if (FinalResult.Count < 24)
            {
                ...
                {
                    var HourExist = FinalResult.Any(p => p.Hours == i);
                    if (HourExist == false)
                    {
                        RequestsCountOverDayDTO temp = new RequestsCountOverDayDTO();
                        temp.Hours = i;
        ...
        public List<RequestsCountOverDayDTO> GetRequestsCountOverDayByDate(DateTime Date)
        {
            ...
            var Result = GetAll().Where(p => p.CreationDate.Value.Year == Date.Year && p.CreationDate.Value.Month == Date.Month && p.CreationDate.Value.Day == Date.Day).GroupBy(p => p.CreationDate.Value.Hour).Select(group => new { Hours = group.Key, RequestsCount = group.Count() }).OrderBy(p => p.Hours).ToList();
            List<RequestsCountOverDayDTO> FinalResult = new List<RequestsCountOverDayDTO>();
            foreach (var item in Result)
            {
                RequestsCountOverDayDTO temp = new RequestsCountOverDayDTO()
