...
namespace HRMS.BLL.Logic.Tables
{
    public class RequestBLL : Repository<Request>
    {
            ...
            out Request Request, out bool CheckMailEnabled, out bool IsAutoApprove)
        {
            ...
            try
            {
                ...
                {
                    SetRequestBackToWorkDate(Request, out backtoworkdate, out OffDays, out OffHours, out NumberOfOffUnit, LeaveType,User.CompanyFK.Value);
...

                    {
                        ...
                        {
                            if (CheckIfOfficialDayorWeekend(Request.DurationFrom, Request.DurationTo, User.CompanyFK.Value))
                            {
                                IsValid = false;
                        ...
                        {
                            IsValid = false;
                            Messages = Messages + "-Please Review Start Or/And End Date ##$$";
                        ...
                        if (CheckAbsenceRequestInThePastValidation(Request, out NumberOfDaysInThePastAllowed,User.CompanyFK.Value))
                        {
                            IsValid = false;
                            Messages = Messages + "-You Can't Request This Leave Type More  Than " + NumberOfDaysInThePastAllowed + " Days in The Past ##$$";

                        }
                        if (CheckAbsenceLongerThanMustBeRequestedBeforeValidation(Request, NumberOfOffUnit, out NumberOfDaysMustBeRequestedBefor, out NumberOfUnit,User.CompanyFK.Value))
                        {
                            IsValid = false;
        ...
        public void CheckDays(DateTime start, DateTime end, out List<DateTime> offdays, out DateTime backtoworkdate, out decimal NumberOfOffUnit,int UserCompanyID)
        {
            try
            {
                ...
                var CompanyOffDays = companyOfficialHolidayBLL.GetAll().Where(p => p.CompanyFK == UserCompanyID).ToList();
                ...
                for (int j = 0; j < daysInWeek.Count; j++)
                {
                    ...
                    foreach (var item in CompanyOffDays)
                    {
                        if (daysInWeek.Count()-1>= j) {
                            if ((int)daysInWeek[j] == item.OffDayOfWeekID)
                            {
        ...
        public void CheckDaysIncludingPublicHolidays(DateTime start, DateTime end, out List<DateTime> offdays, out DateTime backtoworkdate, out decimal NumberOfOffUnit)
        {
            try
            {
                var dateInterval = new List<DateTime>();
        ...
        public void CheckRequestLength(DateTime start, DateTime end, out decimal NumberOfOffUnit)
        {
            try
            {
                var dateInterval = new List<DateTime>();
        ...
        public void RequestLengthIncludingPublicHolidays(DateTime start, DateTime end, out decimal NumberOfOffUnit)
        {
            try
            {
                var dateInterval = new List<DateTime>();
        ...
        public DateTime Backtoworkdate(List<DateTime> offdays, DateTime start, DateTime end)
        {
            ...
            else
            {
                ...
                Valid: if (backtoworkdate.DayOfWeek == DayOfWeek.Friday)
                    ...
                    {
                        if (Holidays[i].HolidayDate.Value.Date == backtoworkdate)
                        {
                            backtoworkdate = backtoworkdate.AddDays(1);
                            goto Valid;
                        }
        ...
        public void SetRequestBackToWorkDate(Request Request, out DateTime backtoworkdate, out List<DateTime> OffDays, out List<RequestHandlerHours> OffHours, out decimal NumberOfOffUnit, LeaveType LeaveType,int UserCompanyID)
        {
            OffHours = new List<RequestHandlerHours>();

            if (Request.LeaveDurationUnitFK == (int)LeaveDurationUnit.Hours)
            {
                TimeSpan DayEnd = (TimeSpan) companyBLL.Find(x=>x.CompanyID== UserCompanyID).FirstOrDefault().OperationTo;
                ...
                else
                {
                    ...
                    backtoworkdate = Backtoworkdate(OffDays, Request.DurationFrom, Request.DurationFrom);
            ...
            else
            {
                ...
                else if (Request.PartialDurationUnitFK == (int)StaticEnums.LeaveTypePartialDurationUnit.PMHalfDay)
                {
                    ...
                    CheckDays(Request.DurationFrom, Request.DurationTo, out OffDays, out backtoworkdate, out NumberOfOffUnit,UserCompanyID);
                    NumberOfOffUnit = (decimal)0.5;
                }
                ...
                else
                {
                    ...
                    {
                        CheckDaysIncludingPublicHolidays(Request.DurationFrom, Request.DurationTo, out OffDays, out backtoworkdate, out NumberOfOffUnit);
                    }
                }

            }
        }
        public bool CheckIfOfficialDayorWeekend(DateTime startdate, DateTime enddate,int UserCompanyFK)
        {
            ...
            foreach (var item in CompanyOffDays)
            {
                if ((int)startdate.DayOfWeek ==item.OffDayOfWeekID || (int) startdate.DayOfWeek == item.OffDayOfWeekID)
                {
                    isitholiday = true;
                }

                if ((int)enddate.DayOfWeek == item.OffDayOfWeekID || (int)enddate.DayOfWeek == item.OffDayOfWeekID)
                {
                    isitholiday = true;
                }
            }
        ...
        public bool CheckAbsenceRequestInThePastValidation(Request Request, out decimal NumberOfDaysInThePastAllowed,int UserCompanyID)
        {
             ...
             x.IsActive == true && x.IsDeleted == false).Count() > 0)
            {
             ...
             x.IsActive == true && x.IsDeleted == false).FirstOrDefault();
                ...
                CheckDays(Request.DurationFrom, (DateTime)Request.CreationDate, out OffDays,out BackToWorkDate, out NumberOfOffDays, UserCompanyID);
        ...
        public bool CheckAbsenceLongerThanMustBeRequestedBeforeValidation(Request Request, decimal NumberOfOffUnit, out decimal NumberOfDaysMustBeRequestedBefore, out decimal NumberOfUnit,int UserCompanyID)
        {
            if (NumberOfOffUnit > 0)
            {
                 ...
                 x.IsActive == true && x.IsDeleted == false).Count() > 0 && Request.DurationFrom >= CreationOfRequestDate.Date)
                {
                 ...
                 x.IsActive == true && x.IsDeleted == false).FirstOrDefault();
                    ...
                    CheckDays(CreationOfRequestDate, Request.DurationFrom, out OffDays, out BackToWorkDays, out CountOffDays, UserCompanyID);
    ...
    ,
        ...
        {
            var request = Find(r => r.RequestID == id && r.RequesStatusFK==(int)RequestStatus.Approved).FirstOrDefault();
            if (request != null)
                return true;
            else
                return false;
           ...
           new RequestHistory
           {
            ...
            if (request.RequesStatusFK == (int)RequestStatus.Approved)
            {
                 ...
                 if (RemaningRequestHandlerDays.Count() > 0)
                {
                    ...
                    out NumberOfOffUnit, LeaveType, Requester.CompanyFK.Value);
                    request.BackToWork = backtoworkdate;
                    request.RequestDuration = NumberOfOffUnit;
                }
                //Edit Request DATA
        ...
        public void DeletePartialPeriodFromRequestHours(RequestDeletionDTO RequestDeletion)
        {
            ...
            if (request.RequesStatusFK == (int)RequestStatus.Approved)
            {
                ...
                if (RemaningRequestHandlerHours.Count() > 0)
                {
                    ...
                    decimal NumberOfOffUnit;
                    SetRequestBackToWorkDate(request, out backtoworkdate, out OffDays, out OffHours, out NumberOfOffUnit, LeaveType, UserDeleter.CompanyFK.Value);
                    request.BackToWork = backtoworkdate;
                    request.RequestDuration = NumberOfOffUnit;
