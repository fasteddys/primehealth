    ...
    public class ApprovalFlowBLL : Repository<ApprovalFlow> 
    {
        ...
        public List<UserDTO> GetUserApprovedByManager(int UserIDOfManager)
        {
            ...
            //{
                ...
                ListUsers.AddRange(userBLL.Find(x => x.DirectManagerFK == ManagerID && x.DirectManagerFK != null && x.IsActive == true));
                ...
                ListUsers.AddRange(userBLL.Find(x => SubDepartmentIDs.Contains((int)x.SubDepartmentFK) && x.IsActive == true));
                ...
                ListUsers.AddRange(userBLL.Find(x => ListApprovalFlowIDs.Contains((int)x.ApprovalFlowFK) && x.IsActive == true));
                foreach (var item in ListUsers.Where(x => x.IsDeleted == false&&x.IsActive == true&&x.UserID!= UserIDOfManager))
                {
                if (ListOfUsers.Where(x => x.UserID == item.UserID).Count() == 0)
                {
                    ListOfUsers.Add(new UserDTO
                    {
                        UserName = item.UserName,
                        UserID = item.UserID,
                        AccessControlUserID = item.AccessControlUserFK,
                        WorkingModeID = item.WorkingModeFK,
