...
namespace WarhouseSystem.Application.ItemApp
{
    public class ManageItem : IManageItem
    {
        ...
        {
            IEnumerable<Item> AllItems = unit.itemRepository.GetAllData().Where(x=>x.IsDeleted == false);
        ...
        public bool UpdateItem(long Id, ItemViewModel model)
        {
            if (Id == 0)
            {
                Item item = map.MapModelToEntity(model);
            ...
            else
            {
                ...
                Utilites.CheckResult = unit.itemRepository.Modify(model.Id, item);
                unit.Save();
                return Utilites.CheckResult;
            }
        }
